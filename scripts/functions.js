...
function removeCNBCPlaceholder(output) {
  let placeHolderHead = output.substring(
    output.indexOf("<!-- CNBC_META_PLACEHOLDER:START -->"),
    output.indexOf("<!-- CNBC_META_PLACEHOLDER:END -->") +
      "<!-- CNBC_META_PLACEHOLDER:END -->".length
  );
  let result = output.replace(placeHolderHead, "");
  let placeHolderHeader = result.substring(
    result.indexOf("<!-- CNBC_PLACEHOLDER:START -->"),
    result.indexOf("<!-- CNBC_PLACEHOLDER:END -->") +
      "<!-- CNBC_PLACEHOLDER:END -->".length
  );
  result = result.replace(placeHolderHeader, "");
...
function cleanSVG(baseOutput) {
  ...
  for (let i = 0; i < document.getElementsByTagName("svg").length; i++) {
    document.getElementsByTagName("svg").item(i).removeAttribute("id");
  }

  for (let i = 0; i < document.getElementsByTagName("path").length; i++) {
    document.getElementsByTagName("path").item(i).removeAttribute("id");
  }

  for (let i = 0; i < document.getElementsByTagName("g").length; i++) {
    document.getElementsByTagName("g").item(i).removeAttribute("id");
  }
